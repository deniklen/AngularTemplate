...
import { MenuService } from 'src/app/interaction/app.menu.service';
import { AdministrationMainComponent } from '../main/administration.main.component';

@Component({
    /* tslint:disable:component-selector */
    selector: '[administration-menuitem]',
		...
		<ng-container>
			...
			<ul *ngIf="(item.items && active) && item.visible !== false" [@children]="(active ? 'visibleAnimated' : 'hiddenAnimated')" role="menu">
				<ng-template ngFor let-child let-i="index" [ngForOf]="item.items">
					<li administration-menuitem [item]="child" [index]="i" [parentKey]="key" [class]="child.badgeClass" role="none"></li>
				</ng-template>
			</ul>
		</ng-container>
    `,
...
export class AdministrationMenuitemComponent implements OnInit, OnDestroy {
    ...
    constructor(public administration: AdministrationMainComponent, public router: Router, private cd: ChangeDetectorRef, private menuService: MenuService) {
    ...
    itemClick(event: Event) {
        ...
        if (this.item.items) {
            ...
            // hide overlay menus
            this.administration.menuActiveMobile = false;

            if (this.administration.isDesktop() && this.administration.isOverlay()) {
                this.administration.menuInactiveDesktop = true;
            }
        }
    }
